...
public static class TextProcessing
{
	...
	public static void MergeDeletedUserPseudonyms(JsonFile jsonFile)
	{
		foreach (var participant in jsonFile.Participants)
		{
			if (Constants.DeletedPseudonyms.Contains(participant.Name))
			{
				participant.Name = Constants.DeletedValue;
			}
		}

		foreach (var message in jsonFile.Messages)
		{
			if (Constants.DeletedPseudonyms.Contains(message.SenderName))
			{
				message.SenderName = Constants.DeletedValue;
			}

			foreach(var user in message.Users)
			{
				if (Constants.DeletedPseudonyms.Contains(user.Name))
				{
					user.Name = Constants.DeletedValue;
				}
			}
		}
	}
